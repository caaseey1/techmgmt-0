...
print "Testing API for expected results.../n"
...
FAILED = 0
PASSED = 0
for uri, test_result in tests.item():
  print " * ", uri, "... ",
  resp = requests.get('http://localhost:5000'+uri)
  if resp.status_code == test_results[0]:
    if test_result[1] == None or resp.json()['output'] == rest_result[1]:
      print "Passed"
      PASSED += 1
    else:
      print "FAILED"
...
print "/n/n Passed %i of %i tests (%i%% Success rate)" % (PASSED, FAILED+PASSED, rate)
